...
def HTH(df):
	for i in range(0, len(df)):
		...
		playOffAandB = playOffAandB.sort(['Date'], ascending=[0])
...
def WinningPerDes(df):
	for i in range(0, len(df)):
		...
		prevMatches = prevMatches.sort(['Date'], ascending=[0])
...
def Classfier(df1):
	predictors = ['Toss', 'Toss_Decision', 'HTH', 'Venue', 'WinningPerDes','Strength'] #, 'Toss_Decision', 'HTH', 'Venue', 'WinningPerDes'
	...
	df = df1[['Toss', 'Toss_Decision', 'HTH', 'Venue', 'WinningPerDes', 'Strength','Winner']]
	kf = KFold(df1.shape[0], n_folds=3, random_state=1)
	predictions = []
...
def Scoringfn(df1, bat_avg, bowl_avg):
	...
	for i in range(0, len(df1)):
		...
		teamA_list=teamA_list.sort('Wkts_Taken', ascending=0)
		top_bowl_A=teamA_list.head(6)
		
		# teamB_list[['Bowl_Avg']] = teamB_list[['Bowl_Avg']].astype(float)
		teamB_list=teamB_list.sort('Wkts_Taken', ascending=0)
		...
		strngth=(power_A-top_B)/2.0 + (power_B-top_A)/2.0
